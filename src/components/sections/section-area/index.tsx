...
const SectionArea = () => {
  const openWhatsapp = () => {
    window.open(
      "https://wa.me/6281234567890?text=Halo%20Andi%20Teknik%2C%20saya%20ingin%20tanya%20area%20layanan",
      "_blank",
    );
  };
  ...
  return (
    <div className={cn.main} id="area">
      ...
      >
        <motion.p className={cn.subtitle} variants={fadeInUp}>JANGKAUAN KAMI</motion.p>
        <motion.h2 className={cn.title} variants={fadeInUp}>Area Layanan</motion.h2>
        <motion.div className={cn.line} variants={fadeInUp} />
        <motion.p className={cn.desc} variants={fadeInUp}>
          Kami melayani seluruh wilayah Jabodetabek. Teknisi kami siap datang ke
          lokasi Anda dalam waktu singkat untuk menyelesaikan masalah AC.
        </motion.p>
      </motion.div>
      ...
      >
        <div className={cn.notInListCard}>
          ...
          <p className={cn.notInListDesc}>
            Hubungi kami untuk mengetahui apakah area Anda masuk dalam jangkauan
            layanan kami. Kami terus memperluas area layanan.
          </p>
          <motion.button
            className={cn.notInListBtn}
